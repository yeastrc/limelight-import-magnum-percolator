...
public class PercolatorResultsReader {
	 ...
	 * @throws Throwable
	 */
	public static PercolatorResults getPercolatorResults( File file, boolean isOpenMods, boolean isSubSearches, boolean importDecoys ) throws Throwable {
		...
		Map<ReportedPeptide, PercolatorPeptideResult> peptideResults = getPercolatorPeptidePSMMap( po, psmIdPSMMap, importDecoys );
	 ...
	 * @throws Exception
	 */
	protected static Map<ReportedPeptide, PercolatorPeptideResult> getPercolatorPeptidePSMMap( IPercolatorOutput po, Map<PercolatorPsmId, PercolatorPSM> psmIdPSMMap, boolean importDecoys ) throws Exception {
	    ...
	    for( IPeptide xpeptide : po.getPeptides().getPeptide() ) {
			...
			// skip this peptide if we're not importing decoys and it only has decoy PSMs
			if(!importDecoys && percolatorPeptideOnlyContainsDecoyPsms(xpeptide, psmIdPSMMap)) {
				continue;
			}
	    	...
	    	Map<SubSearchName, Map<ScanNumber, Collection<PercolatorPSM>>> psmsForPeptide = getPercolatorPSMsForPeptide( xpeptide, psmIdPSMMap, importDecoys );
	...
	/**
	 * Return true if this percolator peptide only contains decoy PSMs
	 *
	 * @param xpeptide
	 * @param psmIdPSMMap
	 * @return
	 * @throws Exception
	 */
	private static boolean percolatorPeptideOnlyContainsDecoyPsms(IPeptide xpeptide, Map<PercolatorPsmId, PercolatorPSM> psmIdPSMMap ) throws Exception {
		...
		for( String psmId : xpeptide.getPsmIds().getPsmId() ) {
			...
			PercolatorPSM percolatorPSM = psmIdPSMMap.get( percolatorPSMId );
			if(!(percolatorPSM.isDecoy())) {
				return false;
			}
		}
	 ...
	 * @throws Exception
	 */
	protected static Map<SubSearchName, Map<ScanNumber, Collection<PercolatorPSM>>> getPercolatorPSMsForPeptide( IPeptide xpeptide, Map<PercolatorPsmId, PercolatorPSM> psmIdPSMMap, boolean importDecoys ) throws Exception {
		...
		for( String psmId : xpeptide.getPsmIds().getPsmId() ) {
			...
			// skip this PSM if it's a decoy and we're not importing decoys
			if(!importDecoys && psm.isDecoy()) {
				continue;
			}
	...
	protected static Map<PercolatorPsmId, PercolatorPSM> getPercolatorPSMs( IPercolatorOutput po, boolean isOpenMods, boolean isSubSearches ) {
	    ...
	    for( IPsm xpsm : po.getPsms().getPsm() ) {
			...
			psm.setDecoy(xpsm.isDecoy() != null && xpsm.isDecoy());
