...
import org.yeastrc.emozi.emozi_import.api.xml_dto.MatchedProtein;
import org.yeastrc.emozi.emozi_import.api.xml_dto.MatchedProteinLabel;
import org.yeastrc.emozi.emozi_import.api.xml_dto.MatchedProteins;
import org.yeastrc.emozi.xml.magnum.objects.ParsedPeptide;
import org.yeastrc.emozi.xml.magnum.objects.PercolatorPeptide;
import org.yeastrc.emozi.xml.magnum.utils.ProteinInferenceUtils;
...
public class MatchedProteinsBuilder {
	 ...
	 * to the emozi xml document in the matched proteins sectioni.
	 * 
	 * @param emoziInputRoot
	 * @param fastaFile
	 * @param decoyIdentifiers
	 * @throws Exception
	 */
	public void buildMatchedProteins( EmoziInput emoziInputRoot, File fastaFile, Collection<PercolatorPeptide> percolatorPeptides ) throws Exception {
		...
		buildAndAddMatchedProteinsToXML( emoziInputRoot, proteins );
	 ...
	 * @throws Exception
	 */
	private void buildAndAddMatchedProteinsToXML( EmoziInput emoziInputRoot, Map<String, Collection<FastaProteinAnnotation>> proteins ) throws Exception {
		
		MatchedProteins xmlMatchedProteins = new MatchedProteins();
		emoziInputRoot.setMatchedProteins( xmlMatchedProteins );
	...
	private Collection<String> getNakedPeptideSequences( Collection< PercolatorPeptide > percolatorPeptides ) {
		
		Collection<String> nakedSequences = new HashSet<>();
		
		for( PercolatorPeptide percolatorPeptide : percolatorPeptides ) {
			ParsedPeptide parsedPeptide = ReportedPeptideParsingUtils.parsePeptide( percolatorPeptide.getReportedPeptide() );
			nakedSequences.add( parsedPeptide.getNakedSequence() );
		}
